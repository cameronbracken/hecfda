...
class ValueUncertainty : public hecfda::statistics::Validation {
   ...
   private:
    ...
    // checked fields BY VALUE, not `[this]`. ValueUncertainty is held BY VALUE inside the
    // move-only OccupancyType and is move-assigned by OccupancyTypeBuilder, so a `[this]`-
    ...
    void add_rules() {
        ...
        add_single_property_rule(
            ...
            [value = std_or_min_]() { return value >= 0; },
            "The percent of inventory value must be greaeter than or equal to zero.",
            hecfda::statistics::ErrorLevel::Fatal);
        add_single_property_rule(
            "_PercentOfInventoryValueMax", [value = max_]() { return value >= 100; },
